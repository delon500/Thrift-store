    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6">
        <div className="flex items-start justify-between">
          <h2 className="text-xl font-black text-teal-600">Payment</h2>
          <button
            type="button"
            onClick={onClose}
...
const PaymentsPage = () => {
  ...
  const inputClass =
    "rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm outline-none focus:border-teal-600";

  return (
    <div className="p-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-black text-teal-600">Payments</h1>
        <p className="text-sm font-medium text-gray-500">
          Confirm payments, reconcile, and investigate failures &amp; refunds.
        </p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <SummaryCard
          label="Paid"
          value={`R${money(summary.total_paid)}`}
          accent="text-teal-600"
        />
        <SummaryCard
          label="Refunded"
          value={`R${money(summary.total_refunded)}`}
          accent="text-orange-600"
        />
        <SummaryCard
          label="Failed"
          value={summary.failed_count || 0}
          accent={summary.failed_count ? "text-red-600" : "text-gray-800"}
        />
      ...
      <section className="mt-6 overflow-hidden rounded-xl border border-gray-200 bg-white">
        {isLoading ? (
          <p className="p-5 text-gray-500">Loading payments...</p>
        ) : payments.length === 0 ? (
          <p className="p-5 text-gray-500">No payments found.</p>
        ) : (
